...
> [!IMPORTANT]
> В libcamera внесён локальный патч, который публикует аппаратный номер кадра
> Unicam в metadata каждого camera request как
> `controls::rpi::UnicamSequence`. Для этого приложению не нужно запрашивать
> RAW stream: номер берётся из Bayer-буфера, поданного в VC4 ISP. Это
> 32-битный V4L2-счётчик с начала стриминга Unicam, поэтому он может
> переполняться и содержать пропуски на старте или при отброшенных кадрах.
